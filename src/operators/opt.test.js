...
describe('opt', () => {
  it('returns the value of a property or an index of an array', () => {
    for (const _ of range(10)) {
      ...
      for (const i of range(a.length)) {
        expect(opt(a)(i)).to.equal(a?.[i])
      }
      expect(opt(a)('length')).to.equal(a?.length)
    }
  })
  it('returns undefined if its object does not exist', () => {
    ...
    expect(opt(a)(1)).to.equal(a?.[1]).and.to.be.undefined
    expect(opt(b)('length')).to.equal(b?.length).and.to.be.undefined
